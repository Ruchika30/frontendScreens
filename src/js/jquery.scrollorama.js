...
(function ($) {
  $.scrollorama = function (options) {
    ...
    const defaults = { offset: 0, enablePin: true };
    ...
    // PRIVATE FUNCTIONS
    function init() {
      let i; let block; let didScroll; let
        marginTop = false;
      ...
      for (i = 0; i < scrollorama.settings.blocks.length; i++) {
        ...
        blocks.push({
          block,
          top: block.offset().top - (!marginTop ? parseInt(marginTop, 10) : 0),
          pin: 0,
          animations: []
        });
      }
    ...
    function onScrollorama() {
      ...
      const currBlockIndex = getCurrBlockIndex(scrollTop);
      let i; let j; let anim; let startAnimPos; let endAnimPos; let animPercent; let
        animVal;
    ...
    scrollorama.getScrollpoints = function () {
      const scrollpoints = []; let i; let j; let anim;
      for (i = 0; i < blocks.length; i++) {
